...
class CamoufoxScrapeScriptTest < ActiveSupport::TestCase
  ...
  FAKE_SYNC_API = <<~PYTHON
    ...
    class _Page:
        ...
        def wait_for_load_state(self, state="load", **kwargs):
            ...
            _dump()
            if LOAD_RAISES:
                raise RuntimeError("Timeout 15000ms exceeded waiting for load")
  ...
  def run_script(url: REQUESTED_URL, content_length: 6_000, final_url: FINAL_URL, load_raises: false, args: [])
    Dir.mktmpdir do |dir|
  ...
  test "página lenta não vira erro: timeout do load é degradação, não exceção" do
