...
| 2026-09-26 | Regra dura escrita no AGENTS.md (#4, backoff 6-12h em 403) e INERTE no caminho que mais importa: o canal do Reddit navegou 65x em 90 min num alvo que responde 403 em 0,025s (card t_f63b3613) | Três furos independentes, e **qualquer um deles sozinho bastava**. (1) **O backoff por sintoma não tem call site onde o fato acontece**: `ScrapingServices::RateLimitHandler` (`lib/scraping/rate_limit_handler.rb`, `DEFAULT_BACKOFF` 6h / `HEAVY_BACKOFF` 12h) tem **ZERO** chamadores em `lib/fetcher` — medido por contagem de arquivo. (2) **O cooldown por alvo existe e funciona, mas só é lido no caminho errado**: `BotDetection.cooldown!` (`lib/fetcher/bot_detection.rb:40-59`, TTL de 6-12h) grava e `cooldown?` responde — MEDIDO gravando 9h de TTL — mas quem o consulta é `PageFetcher#call:347`, e o caminho de canal entra por `BrowserSession.with_page` (`browser_session.rb:74`), que não cita `BotDetection` (verificado por leitura do arquivo). (3) **O 403 nunca chega ao código como 403**: 6/6 chamadas reais de `Reddit.search` e `Reddit.call` produziram `RenderTimeout(35s)`, ZERO `PageFailed`/`SearchFailed`, e `BotDetection.cooldown_for("old.reddit.com")` continuou `nil`. O caminho de canal não testa status HTTP — `browser_session.rb:138` lê `page.network.response&.status` **só para LOG**, e no log real saiu **vazio em 53 das 65** navegações. Pior: o corpo do 403 **tem** os marcadores que o canal procura (`whoa there, pardner` e `blocked due to a network policy`, medido), mas o `goto_limit` de 15s estoura antes, o `body_check` volta vazio e o `raise RenderTimeout` (linha 134) acontece **antes do `yield`** (linha 156) — o `EXTRACT_JS` nunca lê nada. **O balde de 2/min NÃO é o gargalo e NÃO está quebrado**: estourou na 3ª de 6 (p8) e em 10 de 12 threads, e a janela **desliza** com a atividade (o `increment` renova o TTL — medido por relógio de parede; a primeira versão da prova rotulava "t=35s" sem dormir e mediu rajada, o que inverteu o achado). Ele nunca disparou em produção porque o ritmo real é 1,2/min, e as 16 `page_fetch` do pico T18:45 eram de 13 hosts — só 2 eram reddit. **`thread_comments` não cobra NENHUM balde**, então o alvo real (um só: `old.reddit.com`) é contado em três baldes distintos — `www.reddit.com` (Extract), `old.reddit.com` (busca) e nenhum (thread) | **AINDA NÃO CORRIGIDO — o card mandou medir a causa primeiro.** O conserto deveria agir num ponto só: `BrowserSession.with_page`, ANTES do `go_to`, e precisa de duas coisas que não existem lá: checar o cooldown por alvo e classificar o 403 como 403 em vez de timeout. **Achado lateral que corrige um FATO do card**: o 403 foi atribuído ao `SCRAPING_PROXY`, mas o Chrome do container **não tem `--proxy-server`** (flags medidas) e `ferrum.rb:89` só aplica a flag no Chromium que o FERRUM cria — em produção o processo já existe e o Ferrum só conecta por CDP. O proxy é lido no ferrum e em 3 jobs (twitter/instagram/youtube), nenhum deles no caminho do Reddit; o bloqueio é do IP da VM (datacenter da OCI). Mesma conclusão prática (faixa de datacenter), causa nomeada errada. **Regra geral: backoff escrito no AGENTS.md sem call site no caminho quente é letra morta — e um 403 que se manifesta como TIMEOUT não aciona backoff nenhum, porque backoff é por SINTOMA NOMEADO. Antes de jurar que um teto protege, medir se o código que o escreve é o código que o caminho percorre.** Provas: `scripts/proofs/reddit_403_gap_p2..p11.rb` + `reddit_403_backoff_gap_proof.rb`; relatório em `scripts/proofs/RELATORIO-reddit-403-backoff.md`; commit `6980313`. |
