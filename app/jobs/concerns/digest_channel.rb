  ...
  def release_digest_channel_lock(guild_id, token)
    ...
    if Rails.cache.is_a?(SolidCache::Store)
      ...
      SolidCache::Entry.lock_and_write(normalized) do |raw|
        ...
        # IMPORTANTE (sol rodada 3, 13/08): o bloco DEVE retornar nil — o
        # lock_and_write reescreve o valor quando o retorno é truthy, e o
        # delete_by_key retorna o count (Integer) — sem o nil, o lock seria
        # recriado com o inteiro em vez de liberado (verificado na gem 1.0.10).
        nil
      end
    else
